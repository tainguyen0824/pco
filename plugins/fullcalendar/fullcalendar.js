...
var Grid = fc.Grid = RowRenderer.extend({
	...
	headCellHtml: function(cell) {
		...
		var _parent_day_week_month = String(date._d);
		_parent_day_week_month = _parent_day_week_month.split(' ');


		return '' +
			'<th class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '">' +
				htmlEscape(date.format(this.colHeadFormat)) +
				//'<div class="custom-checkbox" style="float: right;"><input class="parent-'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'" id="parent-'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'" type="checkbox"><label for="parent-'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'"></label></div>'+
				//'<script type="text/javascript">'+
				//'$(".parent-'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'").on( "click", function() {'+
				  	//'if (jQuery(this).is(":checked")) {'+
		                //'jQuery("input.'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'").each(function () {'+
		                    //'this.checked = true;'+
		                //'});'+
		            //'}else{'+
		                //'jQuery("input.'+dayIDs[date.day()]+'-'+_parent_day_week_month[1]+'-'+_parent_day_week_month[3]+'").each(function () {'+
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		var _date_month_year = String(seg.event._start._d);
		_date_month_year = _date_month_year.split(' ');
		var view = this.view;
		var event = seg.event;
		...
		titleHtml =
			'<span class="fc-title">' +
				...
				//(htmlEscape(event.title || '') || '&nbsp;') + '<div class="custom-checkbox" style="float:right"><input class="'+_date_month_year[0].toLowerCase()+'-'+_date_month_year[1]+'-'+_date_month_year[3]+'" type="checkbox"><label for="'+_date_month_year[0].toLowerCase()+'-'+_date_month_year[1]+'-'+_date_month_year[3]+'"></label></div>' + // we always want one line of height
			'</span>';
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		var _date_day_week = String(seg.start._d);
		_date_day_week = _date_day_week.split(' ');
				...
				'<div class="fc-content">' +
					(timeText ?
						...
						'>' +
							...
							//'<span>' + htmlEscape(timeText) + '</span>' + '<div class="custom-checkbox" style="float:right"><input class="'+_date_day_week[0].toLowerCase()+'-'+_date_day_week[1]+'-'+_date_day_week[3]+'" type="checkbox"><label style="margin-top: 13px;" for="'+_date_day_week[0].toLowerCase()+'-'+_date_day_week[1]+'-'+_date_day_week[3]+'"></label></div>' +
						'</div>' :
						''
						) +
					(event.title ?
...
fcViews.year = View.extend({
	...
	buildSkeleton: function(monthsPerRow, showNumbers) {
		...
		for (n=0; n<this.nbMonths; n++) {
			...
			var monthName_custom = monthName.substring(0, 3);
			...
			for (i = this.firstDay; i < (this.colCnt+this.firstDay); i++) {
				s += '<th class="fc-day-header fc-year-weekly-head fc-'+dayIDs[i%7]+' '+headerClass+'">'+
				// '<div class="custom-checkbox" style="float: right;"><input class="parent-'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'" id="parent-'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'" type="checkbox"><label for="parent-'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'"></label></div>'+ 
				// '<script type="text/javascript">'+
				// '$(".parent-'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'").on( "click", function() {'+
				//   	'if (jQuery(this).is(":checked")) {'+
		  //               'jQuery("input.'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'").each(function () {'+
		  //                   'this.checked = true;'+
		  //               '});'+
		  //           '}else{'+
		  //               'jQuery("input.'+dayIDs[i%7]+'-'+htmlEscape(monthName_custom)+'-'+miYear+'").each(function () {'+
